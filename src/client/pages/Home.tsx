...
const Home = () => {
    ...
    const handleTodoDelete = async (id: string) => {
        ...
        try {
            const { data } = await axios.delete<TBasicRes>(`${server}/task/${user?._id}/${id}`, {
                withCredentials: true,
            })
            toast.success(data.message)
            setRefresh((prevVal) => !prevVal)
    ...
    return (
        <>
            <div className='m-auto mt-24 w-2/4 rounded-lg bg-gradient-to-br from-[rgba(255,255,255,0.05)] to-transparent p-6'>
                <form onSubmit={handleSubmit} className='flex flex-col gap-4 bg-transparent'>
                    ...
                    <button
                        type='submit'
                        disabled={loading}
                        className='m-auto w-full rounded-lg border-2 bg-white px-3 py-2 text-xl text-black flex justify-center items-center'>
