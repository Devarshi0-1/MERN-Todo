...
const AdminUserInfo = ({ user, handleUserDelete }: TProp) => {
    ...
    const { error, data }: TFetchGetTasks = useFetchGet(`/admin/tasks/all/${user?._id}`, refresh)

    if (error.error) toast.error(error.message)

    const handleTodoDelete = async (id: string) => {
        try {
            const { data } = await axios.delete<TBasicRes>(`${server}/admin/task/${id}`, {
    ...
    return (
        <>
            <div className='relative grid grid-cols-[1fr_1fr_1fr_1fr_1fr_0.1fr] items-center rounded-lg bg-gradient-to-br from-[rgba(255,255,255,0.05)] to-transparent px-4 text-white'>
            ...
            {data.tasks?.length > 0 && (
                <div
                    className={`${
                        isExpand ? '' : 'hidden'
                    } mx-auto w-11/12 rounded-lg bg-gradient-to-br from-[rgba(255,255,255,0.05)] to-transparent text-white`}>
                    {data.tasks.map((task) => (
                        <AdminTodo
                            key={task._id}
                            id={task._id}
